<?php
	...
	// TO DO : prendre en compte le fait qu'un point n'apparaisse bien que 3 fois
	function computeWifiSignal($tabSignals){
		
		$tab = array();
		$tabCount = array();
		// Count of ID
		// $tabCount = array_count_values(array_column($tabSignals,"id"));
	...
	function getTop5($tab){
		...
		var_dump($tab);
		// var_dump($tab);
		$TOP5 = array_slice($tab,0,5,true);
		
		$result = array();
		foreach($TOP5 as $bssid => $force){
			$result[] = array("bssid" => $bssid, "rssi" => $force);
		}
	...
	function compareWifiSignalsWithHardware($smartphoneSignals){
		//Select Hardware with same signals (BSSID) : SparQL Request
		
		//Get Signals of the Hardware
		
		//Select signals of Smartphone which are the same as Hardware
		
		//Mean of Signals
		//$meanSmartphone = array_sum($foo) / count($foo); 
		
		//Compararison with Threshold
	...
	//Insert data to KB
	//	add hardware : id
	//	add wifi, add bluetooth : RSSI, BSSID +("hardware" id)+("observation")
	//	add footprint :

	//for wifi $result[] = array("bssid" => $bssid, "rssi" => $force);
	...
	function add_footprint($wifi,$hardware,$place){
		...
		//create footprint -- 1
		//connect the footprint with hardware -- 2 3 4
		//connect the footprint with the place -- 5 6 7
		//connect the footprint with the wifi signal result -- 8 9 10 11
		//time when the observation was made -- 12  ??? sent by server or time here
	...
	function find_footprint($wifi){
		...
		{
			//find by the bssid of the signal
